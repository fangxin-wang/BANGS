...
def read_logs_from_directory(directory):
    """Read log_before_12 files from the specified directory and parse their contents."""
    data = {}
...
def extract_info_from_filename(filename):
    ...
    pattern = re.compile(
        r"cautious-(\w+)-(\w+)-itr(\d+)-top(\d+)-seed(\d+)-m(\d+)-(\w+)\.txt"
    )
...
df = df[['Model', 'Dataset', 'Iteration', 'Node Num Each Iteration', 'Seed', 'Multiview', 'Selection Criterion', 'origin_accuracy',
         'best_test_accuracy', 'final_test_accuracy', 'best_acc_early_stopped']]
df.columns = ['Model', 'Dataset', 'Iteration', 'Node Num', 'Seed', 'Multiview', 'Selection Criterion', 'Origin',
...
df_select = df[  (df.Seed.isin([ 813, 814, 815]) ) & (df['Iteration']==40)]
...
res = df_select.groupby(['Dataset','Model','Node Num','Multiview','Selection Criterion'])[[ 'Origin', 'Best Test', 'Early Stopped']].agg(['mean','std','count'])
print(res)